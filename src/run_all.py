...
from .visualization import (
    ...
    plot_metric_sweep_learning_curves,
)
from .config import *
...
def run_complete_pipeline():
    ...
    # Metric sweep learning curves
    print("\n9b. Generating metric sweep learning curves...")
    ...
    for model_name, model in models.items():
        print(f"  -> {model_name}")
        sweeps = plot_metric_sweep_learning_curves(
            model,
            X_full_scaled,
            y,
            model_name,
        ...
        for metric_name, df in sweeps.items():
            df.to_csv(
                model_metrics_dir / f"metric_sweep_{metric_name}.csv", index=False
            )

        commentary = interpret_metric_sweep(model_name, sweeps, focus_metric="f1_macro")
        commentary_lines.append(f"{model_name}: {commentary}")

        with open(model_metrics_dir / "metric_sweep_commentary.txt", "w") as f:
            f.write(commentary + "\n")

    with open(METRICS_DIR / "metric_sweep_overview.txt", "w") as f:
